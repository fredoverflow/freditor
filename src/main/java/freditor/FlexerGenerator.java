...
public class FlexerGenerator {
    ...
    public FlexerGenerator(int lastNegative, int lastPositive) {
        identifierCall = "identifier(input)";
        negative = lastNegative;
        positive = lastPositive;
    }

    public FlexerGenerator withIdentifierCall(String identifierCall) {
    ...
    public void generateTokens(String... tokens) {
        ...
        autoGenerated();
        generateTransitionCases(true, identifierCall);
        generateTransitionCases(false, "END");
    }
    ...
    private void generateTransitionCases(boolean filterOnKeywords, String tokenReturnValue) {
        ...
        for (Map.Entry<String, String> entry : transitions.entrySet()) {
            ...
            if (isKeywordPrefix(prefix) == filterOnKeywords) {
                if (inputs.isEmpty()) {
                    ...
                    generateTransitionCase(prefix, inputs, tokenReturnValue);
                }
            }
        }
    ...
    private void generateTransitionCase(String prefix, String inputs, String tokenReturnValue) {
        System.out.print("case " + stateName(prefix) + ": return ");
        final int len = inputs.length();
        for (int i = 0; i < len; ++i) {
            char input = inputs.charAt(i);
            System.out.print("input == '" + input + "' ? " + stateName(prefix + input) + " : ");
        }
        System.out.println(tokenReturnValue + ";");
    }
